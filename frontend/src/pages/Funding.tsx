...
function LineChart({
  ...
  const xs = data.map(
    (_, i) => PAD.left + (i / Math.max(data.length - 1, 1)) * inner.w,
  );
  const ys = data.map(
    (d) => PAD.top + (1 - (d.rate_ann - minR) / rangeR) * inner.h,
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <h1 className="text-base font-semibold text-zinc-100 mr-1">
          Funding rates
        </h1>
      ...
      {dataUpdatedAt > 0 && (
        <p className="text-[10px] text-zinc-700">
          Mis à jour à {new Date(dataUpdatedAt).toLocaleTimeString('fr-FR')} · refresh auto toutes les 15 min
        </p>
      )}
          ...
          <div className="rounded border border-zinc-800 bg-zinc-900/30 px-4 py-3 text-xs text-zinc-500 space-y-1">
            <p>
              <span className="text-zinc-400 font-medium">
                Coût de hedge (perp short BTC) :
              </span>{" "}
              funding positif = tu{" "}
              <span className="text-emerald-400">reçois</span> du funding en
              shortant le perp. Funding négatif = tu{" "}
              <span className="text-red-400">paies</span>.
            </p>
            <p>
              Moyenne sur {days}j :{" "}
              <span className={rateColor(s.avg)}>{fmt(s.avg, 2)}% ann.</span>
              {s.avg >= 0
...
function IndexChart({
  ...
  const ys = data.map(
    (d) => PAD.top + (1 - (d.index_price - minP) / rangeP) * inner.h,
  );
  const points = xs.map((x, i) => `${x},${ys[i]}`).join(" ");
