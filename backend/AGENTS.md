...
│   ├── health.py
│   ├── opportunities.py     # list + detail + /{id}/snapshots (evolution series)
...
├── structured/              # multi-leg structured arb — box spreads (separate from the 1:1 screener)
│   ├── types.py            # Leg, StructuredOpportunityDTO (all Decimal, + spot), settlement_class()
│   ├── strategies/box.py   # detect_boxes() — pure; best-venue-per-leg on each (K1,K2) pair
│   └── screener.py         # StructuredScreener: 1s loop, reads BookCache; maintains
│                           #   structured_opportunities (identity = strategy/underlying/expiry/strikes)
│                           #   + structured_opportunity_snapshots time series (change- or interval-gated)
│                           #   + close sweep each tick → live_status STALE / EXPIRED
├── services/
│   ├── screener.py          # 500ms loop → opportunities (executor workflow `status`) + evolution
│   │                        #   history: live_status (LIVE/STALE/EXPIRED) + opportunity_snapshots
│   │                        #   + close sweep + revive-on-reappear; never touches `status`
│   ├── snapshots.py         # should_snapshot() — shared gate for both screeners' time series
│   ├── comparator.py        # 1:1 port of TS compareOptions (Decimal); Spread carries underlying_price
...
│   │                        #   opportunities + structured_opportunities; *_snapshots time-series tables
...
├── worker.py                # entry-point: WS + screener + alerter + rebalancer + perp_hedger + retention (daily prune: opportunities + *_snapshots + finished structured) + structured screener (if structured.enabled)
...
**`status` vs `live_status`**: the executor owns the workflow `status` (PENDING → APPROVED/REJECTED/EXECUTED). The screener owns `live_status` (LIVE/STALE/EXPIRED) + the `opportunity_snapshots` history + the `peak_*`/`last_seen_at` lifecycle fields — a separate column it updates on every re-detection, never `status`. Its close sweep and revive-on-reappear touch only `live_status`/`closed_at`/`close_reason`.
