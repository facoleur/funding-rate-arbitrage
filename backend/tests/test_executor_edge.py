...
def _inst(
    exchange: str, name: str = "BTC-20260101-30000-C", *, expiry_days: int = 30
) -> Instrument:
    return Instrument(
        exchange=exchange,
...
def _executor(
    a: MockExchange, b: MockExchange, ia: Instrument, ib: Instrument, cfg: AppConfig | None = None
) -> Executor:
    return Executor(
        cfg or AppConfig(),
...
def _pair(
    bids=("100", "10"), asks=("101", "10"), other_bids=("115", "10"), other_asks=("116", "10")
) -> tuple:
    ...
    b = MockExchange(
        "deribit",
        slippage=SlippageModel(
            noise_stdev_bps=0, reject_prob=0, latency_min_sec=0, latency_max_sec=0, rng_seed=2
        ),
    )
...
async def test_apr_dropped_rejects_when_spread_narrows(test_db: str) -> None:
    ...
    a, b, ia, ib = _pair(
        bids=("100", "10"),
        asks=("101", "10"),
        other_bids=("100.05", "10"),
        other_asks=("101.05", "10"),  # tiny spread — apr drops below 10%
    )
    await _make_pending_opp()
    ...
    async with get_session() as sess:
        opp = (await sess.execute(select(Opportunity))).scalars().first()
    assert opp.status == OpportunityStatus.REJECTED
    assert "apr_dropped" in (opp.rejection_reason or "") or "size_too_small" in (
...
async def test_empty_book_rejects(test_db: str) -> None:
    ...
    async with get_session() as sess:
        opp = (await sess.execute(select(Opportunity))).scalars().first()
    assert opp.status == OpportunityStatus.REJECTED
    assert "empty_book" in (opp.rejection_reason or "") or "apr_dropped" in (
        opp.rejection_reason or ""
    )
